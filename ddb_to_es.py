...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Set-up importing to dynamodb')
    ...
    parser.add_argument(
        '--lf', metavar='LF', help='lambda function that posts data to es', required=True)
    parser.add_argument('--esarn', metavar='ESARN',
                        help='event source ARN', required=True)
...
def import_dynamodb_items_to_es(table_name, aws_secret, aws_access, aws_token, aws_region, event_source_arn, lambda_f, scan_limit):
    ...
    session = Session(aws_access_key_id=aws_access, aws_secret_access_key=aws_secret,
    ...
    while True:
        ...
        for i in response["Items"]:
            ...
            ddb_keys = boto3.dynamodb.types.TypeSerializer().serialize(ddb_keys)[
                "M"]
            record = {
